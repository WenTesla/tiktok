...
//  通过两者的用户Id查询最新最新的两者之间的聊天记录 0-接受 1-发送 有点问题
...
func QueryMessageByUserIdAndToUserId(userId int64, toUserId int64) ([]Message, error) {
	...
	result := db.Where("user_id = ? AND to_user_id = ? AND is_withdraw = ?", userId, toUserId, 0).Or("user_id = ? AND to_user_id = ? AND is_withdraw = ?", toUserId, userId, 0).Order("createTime desc").Find(&messages)
	if result.Error != nil {
